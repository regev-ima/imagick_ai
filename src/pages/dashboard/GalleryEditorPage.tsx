...
export default function GalleryEditorPage() {
  ...
  // A coarse ticking clock so time-based expiry of the "processing" indicator
  // actually fires: gallery/edits polling stops the moment status flips to
  // "ready", but the compression tail (and the safety timeout) still need the
  // clock to advance to eventually clear the spinner.
  const [nowTick, setNowTick] = useState(() => Date.now());

  // Per-look: when its most recent edit landed. After the edit webhook writes
  // the row, the edited image's *reduced* thumbnail is still being generated by
  // an async, callback-less compression step — so we keep the look "processing"
  // for a short grace after its last edit to cover that "coming back from
  // compression" tail the user sees.
  const lastEditAtByStyle = useMemo(() => {
    const map: Record<string, number> = {};
    for (const e of galleryImageEdits) {
      if (!e.style_id || !e.created_at) continue;
      const t = new Date(e.created_at).getTime();
      if (!map[e.style_id] || t > map[e.style_id]) map[e.style_id] = t;
    }
    return map;
  }, [galleryImageEdits]);

  // Looks currently being generated — the VIEW sidebar spins their ring and
  // shows "processing…". Unions instant feedback (pendingReEdit), the
  // server-derived edit phase (survives a reload), and a compression grace —
  // BUT is hard-bounded so a stuck job can never spin forever.
  const processingViewStyleIds = useMemo(() => {
    const set = new Set<string>();

    // ── Safety bound: never "processing forever" on a fault ──
    // Use the editor's existing PROGRESS-based stall detector (no forward
    // progress for ~20 min → auto-retry, then "stalled"). A duration cap from
    // the run's start would wrongly kill the spinner on a legitimately slow but
    // healthy re-edit (a large job easily runs many minutes), so we gate on
    // "stuck", not "slow". The compression grace below is separately bounded.
    if (processingStalled) {
      return set;
    }

    // Instant feedback the moment the user hits Apply.
    for (const sid of pendingReEdit?.styleIds ?? []) set.add(sid);

    // Edit phase: a still-in-flight image is missing this look's edit.
    if (gallery?.status === "processing") {
    ...
    // Compression tail: keep the look "processing" briefly after its newest
    // edit lands, while its reduced thumbnails are still being generated.
    const COMPRESSION_GRACE_MS = 90_000;
    for (const style of galleryStylesData) {
      const last = lastEditAtByStyle[style.id];
      if (last != null && nowTick - last < COMPRESSION_GRACE_MS) set.add(style.id);
    }
    return set;
  }, [pendingReEdit, gallery?.status, images, galleryStylesData, imageIdsByStyle, lastEditAtByStyle, processingStalled, nowTick]);

  // Advance the clock while anything is (or was just) processing, then stop —
  // so the grace/cap expire even after gallery polling has gone quiet.
  useEffect(() => {
    const active = gallery?.status === "processing" || processingViewStyleIds.size > 0;
    if (!active) return;
    const t = setInterval(() => setNowTick(Date.now()), 4000);
    return () => clearInterval(t);
  }, [gallery?.status, processingViewStyleIds.size]);
   ...
   <FailedImagesProvider>
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div ref={scrollContainerRef} className="flex-1 min-w-0 overflow-y-auto [scrollbar-gutter:stable]">
        {/* Persistent culling-status banner — live progress while
            'processing', and the REAL failure reason (pipeline_error) with a
            retry button when the run errored. Sticky so it stays in view when
            the user scrolls down through 3000 thumbnails. */}
