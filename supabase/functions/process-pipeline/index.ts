...
serve(async (req: Request) => {
  ...
  try {
    ...
    const facesTodoCount = doFaces ? withUrl.filter((i) => !facesDone.has(i.id)).length : 0;
    {
      ...
      let marker = (gBill as { pipeline_billing?: { ts?: unknown } | null } | null)?.pipeline_billing ?? null;
      // A user-initiated run over a marker from a DEAD previous run (older
      // than 30 min — a live chain re-invokes far more often) — close the old
      // books, then re-reserve for this run. A fresh marker means a run is
      // live right now: leave its books alone, its reservation covers billing.
      ...
      if (!marker && ownerId && (cullTodo.length > 0 || facesTodoCount > 0)) {
        // Atomic bootstrap claim: only the invocation that lands the
        // placeholder reserves — a double-click / racing chain can't
        // double-reserve (the loser sees a non-null marker and skips).
        const bootTs = Date.now();
        const { data: claim } = await admin
          .from("galleries")
          .update({ pipeline_billing: { ts: bootTs, user_id: ownerId, bootstrapping: true } })
          .eq("id", galleryId)
          .is("pipeline_billing", null)
          .select("id");
        if (claim && claim.length > 0) {
          ...
          if (unlimited || needed === 0) {
            await admin.from("galleries")
              .update({ pipeline_billing: { unlimited: true, ts: bootTs, user_id: ownerId } })
              .eq("id", galleryId);
          } else {
            ...
            await admin.from("galleries").update({
              pipeline_billing: {
                ts: bootTs, user_id: ownerId, reserved: needed,
    ...
    const cullOne = async (img: { id: string; original_url: string }) => {
      // Prefer the cheap thumbnail, but the compressed/thumbnail webp are
      // generated by an ASYNC step that runs after the originals upload — so on
      // a fresh gallery the thumbnail may 404 when culling first reaches an
      // image. Fall back to the ORIGINAL (always present post-upload) so scoring
      // still works, exactly like the CLIP/faces pass does. Normal case = one
