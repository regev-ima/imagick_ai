-- =====================================================================
-- Credit Grants: admin-gifted credits with expiration tracking
-- =====================================================================
...
DECLARE
  ...
  v_take      INTEGER;
BEGIN
  -- Update main counter (existing logic + reservation release)
  UPDATE public.user_subscriptions
...
BEGIN
  ...
  LOOP
    IF v_grant.credits_remaining > 0 THEN
      ...
      SET edits_remaining = CASE
            WHEN edits_remaining = -1 THEN -1
            ELSE GREATEST(0, edits_remaining - v_grant.credits_remaining)
          END,
          updated_at = now()
