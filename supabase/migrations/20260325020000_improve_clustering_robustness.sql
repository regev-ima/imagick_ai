...
-- 1. Filter out deleted/non-ready images from clustering
-- 2. Remove inline face_count increment (only final batch calculation)
-- 3. Handle edge case where no faces have vectors
...
BEGIN
  ...
  -- Count detections with vectors, only for images that still exist and are ready
  SELECT COUNT(*) INTO v_total_detections
  FROM face_detections fd
  JOIN gallery_images gi ON gi.id = fd.image_id
  WHERE fd.gallery_id = p_gallery_id
    AND fd.face_vector IS NOT NULL
    AND gi.status = 'ready';

  RAISE LOG 'cluster_gallery_faces: gallery=%, detections=%, threshold=%', p_gallery_id, v_total_detections, p_distance_threshold;

  -- Early exit if no faces to cluster
  IF v_total_detections = 0 THEN
  ...
  -- Iterate over detections with vectors, only for active images, largest face first
  FOR v_detection IN
    SELECT fd.id, fd.image_id, fd.bounding_box, fd.face_vector
    FROM face_detections fd
    JOIN gallery_images gi ON gi.id = fd.image_id
    WHERE fd.gallery_id = p_gallery_id
      AND fd.face_vector IS NOT NULL
      AND gi.status = 'ready'
    ORDER BY (fd.bounding_box->>'width')::float * (fd.bounding_box->>'height')::float DESC
  LOOP
  ...
  -- Single source of truth: recalculate face_count as unique images per cluster
